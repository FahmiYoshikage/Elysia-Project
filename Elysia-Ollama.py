...
# Tidak lagi memerlukan OPENROUTER_API_KEY, tapi biarkan saja untuk saat ini jika ingin beralih lagi
...
# --- Konfigurasi Klien OpenAI untuk MENGGUNAKAN OLLAMA ---
# Perhatikan perubahan URL dan API key
client = openai.OpenAI(
    base_url="http://host.docker.internal:11434/v1",
    api_key="ollama", # Tidak memerlukan API key asli untuk Ollama
)
...
# --- Fungsi LLM Core dengan OLLAMA ---
async def get_llm_response(prompt: str) -> str:
    """
    Mengambil respons dari model Llama 3 yang berjalan di Ollama.
    """
    try:
        completion = client.chat.completions.create(
            # Ganti dengan nama model yang Anda unduh dari Ollama
            model="llama3",
            messages=[
                {
                    "role": "system",
        ...
        # Karena model bisa berbeda, logikanya mungkin tidak memerlukan penghapusan tag
        # Tapi tetap pertahankan sebagai langkah pencegahan.
